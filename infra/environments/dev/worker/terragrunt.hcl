# -----------------------------------------------------------------------------
# dev / worker
# ECS Fargate — small sizing, scales to zero when idle
# -----------------------------------------------------------------------------
locals {
...
# Pull networking and queue outputs so we can wire them in
...
dependency "queue" {
  ...
  mock_outputs = {
    render_queue_url  = "https://sqs.us-east-1.amazonaws.com/111111111111/mock-queue"
    render_queue_arn  = "arn:aws:sqs:us-east-1:111111111111:mock-queue"
    result_queue_url  = "https://sqs.us-east-1.amazonaws.com/111111111111/mock-result-queue"
    result_queue_arn  = "arn:aws:sqs:us-east-1:111111111111:mock-result-queue"
  }
  mock_outputs_allowed_terraform_commands = ["validate", "plan"]
}
...
inputs = {
  ...
  worker_image = "111111111111.dkr.ecr.us-east-1.amazonaws.com/${local.project}-worker:latest"

  # dev sizing: 0.5 vCPU / 1 GB
  worker_cpu    = 512
  worker_memory = 1024

  # Scaling: dev scales to 0 at night
  ...
  # Secrets (create these in Secrets Manager manually or via a secrets module)
  openai_secret_arn = "arn:aws:secretsmanager:us-east-1:111111111111:secret:dev/creative-video/openai-api-key"
  tts_secret_arn    = "arn:aws:secretsmanager:us-east-1:111111111111:secret:dev/creative-video/tts-api-key"
