# -----------------------------------------------------------------------------
# prod / worker
# ECS Fargate — large sizing (2 vCPU / 4 GB), always-on, scales 1–20
# -----------------------------------------------------------------------------
locals {
...
dependency "queue" {
  ...
  mock_outputs = {
    render_queue_url = "https://sqs.us-east-1.amazonaws.com/333333333333/mock-queue"
    render_queue_arn = "arn:aws:sqs:us-east-1:333333333333:mock-queue"
    result_queue_url = "https://sqs.us-east-1.amazonaws.com/333333333333/mock-result-queue"
    result_queue_arn = "arn:aws:sqs:us-east-1:333333333333:mock-result-queue"
  }
  mock_outputs_allowed_terraform_commands = ["validate", "plan"]
}
...
inputs = {
  ...
  worker_image = "333333333333.dkr.ecr.us-east-1.amazonaws.com/${local.project}-worker:latest"

  # prod sizing: 2 vCPU / 4 GB
  worker_cpu    = 2048
  worker_memory = 4096

  # Scaling: prod always has at least 1 worker, up to 20
  worker_desired_count = 1
  worker_min_count     = 1
  worker_max_count     = 20

  # Prod: faster scale-out, slower scale-in
  ...
  # Secrets
  openai_secret_arn = "arn:aws:secretsmanager:us-east-1:333333333333:secret:prod/creative-video/openai-api-key"
  tts_secret_arn    = "arn:aws:secretsmanager:us-east-1:333333333333:secret:prod/creative-video/tts-api-key"
